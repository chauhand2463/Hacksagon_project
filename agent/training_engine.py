...
'''
    ...
    def create_job(
        ...
        if execution_mode == "local" and gpu_info["available"]:
            ...
            # In production, would run in thread
            job["logs"].append(f"Started local GPU training on {gpu_info['name']}")
            job["logs"].append("Note: For demo, training is simulated. In production, would execute actual training.")
            job["status"] = "completed"
            job["completed_at"] = datetime.utcnow().isoformat()
            job["results"] = {
                "status": "completed",
                "message": "Training executed (simulated for demo)",
